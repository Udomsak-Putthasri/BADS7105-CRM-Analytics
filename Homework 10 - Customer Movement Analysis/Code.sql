WITH cust_month as (
    ...
    FROM `supermarket-316814.Supermarket.Supermarket_Dataset`
    where CUST_CODE is not null 
)

SELECT report_month,
    count(distinct case when status = 'New' THEN CUST_CODE ELSE NULL END) as newCustomers,
    count(distinct case when status = 'Repeat' THEN CUST_CODE ELSE NULL END) as repeatCustomers,
    count(distinct case when status = 'Reactivated' THEN CUST_CODE ELSE NULL END) as reactivatedCustomers,
    count(distinct case when status = 'Churn' THEN CUST_CODE ELSE NULL END) as churnCustomers
FROM (
    SELECT CUST_CODE,
